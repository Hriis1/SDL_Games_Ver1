...
void LevelScene::draw()
{
    ...
    if (_developerVision)
    {
        ...
        auto playerPos = _level.getWalkableGridPos(SDL_Point{(int)(_player.getXPos() + _player.getWidth() / 2.0f), (int)(_player.getYPos() + _player.getHeight() / 2.0f)});
    ...
    for (size_t i = 0; i < _ghosts.size(); i++)
    {
        _ghosts[i].get()->render(_window, _level, _camera.x, _camera.y);
    }
...
{
    //_ghosts.emplace_back(std::make_unique<GreenGhost>(_window.renderer, _window.getSDLWindow(), 575.0f, 286.0f, 1000.0f));
    //_ghosts.push_back(std::make_unique<RedGhost>(_window.renderer, _window.getSDLWindow(), 670.0f, 286.0f, 500.0f));
    _ghosts.emplace_back(std::make_unique<OrangeGhost>(_window.renderer, _window.getSDLWindow(), 575.0f, 390.0f, 1000.0f));
    //_ghosts.emplace_back(std::make_unique<BlueGhost>(_window.renderer, _window.getSDLWindow(), 670.0f, 390.0f, 1000.0f));
}
